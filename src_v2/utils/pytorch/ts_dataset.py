...
class TimeSeriesDataset(Dataset):
    def __init__(self,
        ...
        # Store the dates # new line by HL
        self.dates = Y_df['ds'].values # Assuming 'ds' column is in Y_df
    ...
    def _df_to_lists(self, Y_df, S_df, X_df):
        ...
        for i, u_id in enumerate(unique_ids):
            top_row = (Y_df['unique_id'].searchsorted(u_id, 'left')).item() # old asscalar
            bottom_row = (Y_df['unique_id'].searchsorted(u_id, 'right')).item() # old asscalar
    ...
    # new function by HL
    def get_dates_for_batch(self, index, batch_size):
        # Calculate the start and end index for the batch
        start_idx = index * batch_size
        end_idx = start_idx + batch_size

        # Retrieve and return the dates for the batch
        return self.dates[start_idx:end_idx]