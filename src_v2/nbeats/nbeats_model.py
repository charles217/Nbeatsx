...
from src_v2.nbeats.tcn import TemporalConvNet

def filter_input_vars(insample_y, insample_x_t, outsample_x_t, t_cols, include_var_dict,input_size_multiplier, 
                                                   output_size):
    ...
    assert input_size==(input_size_multiplier+1)*output_size, f'input_size {input_size} not correct, output_size={output_size},input_size_multiplier= {input_size_multiplier} '#168+24, f'input_size {input_size} not 168+24'

    #x_t = x_t.reshape(batch_size, n_channels, 8, 24)
    x_t = x_t.reshape(batch_size, n_channels, input_size_multiplier+1, output_size)
...
class NBeatsBlock(nn.Module):
    ...
    def __init__(self, x_t_n_inputs: int, x_s_n_inputs: int, x_s_n_hidden: int, theta_n_dim: int, basis: nn.Module,
                 n_layers: int, theta_n_hidden: list, include_var_dict, t_cols, batch_normalization: bool, dropout_prob: float, 
                 activation: str, input_size_multiplier: int, output_size : int):
        """
        """
        super().__init__()
    ...
    def forward(self, insample_y: t.Tensor, insample_x_t: t.Tensor,
        ...
        if self.include_var_dict is not None:
            insample_y = filter_input_vars(insample_y=insample_y, insample_x_t=insample_x_t, outsample_x_t=outsample_x_t,
                                           t_cols=self.t_cols, include_var_dict=self.include_var_dict, input_size_multiplier = self.input_size_multiplier, 
        ...
        #print('backcast, forecast sizes', backcast.shape, forecast.shape)
