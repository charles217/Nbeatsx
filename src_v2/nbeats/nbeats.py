...
from src_v2.nbeats.nbeats_model import NBeats, NBeatsBlock, IdentityBasis, TrendBasis, SeasonalityBasis
from src_v2.utils.data.datasets.epf import EPF, EPFInfo
from src_v2.utils.pytorch.ts_dataset import TimeSeriesDataset
from src_v2.nbeats.nbeats_model import ExogenousBasisInterpretable, ExogenousBasisWavenet, ExogenousBasisTCN
from src_v2.utils.pytorch.ts_loader import TimeSeriesLoader
from src_v2.utils.pytorch.losses import MAPELoss, MASELoss, SMAPELoss, MSELoss, MAELoss, PinballLoss
from src_v2.utils.numpy.metrics import mae, pinball_loss#, mse, mape
...
class Nbeats(object):
    ...
    def create_stack(self):
        ...
        for i in range(len(self.stack_types)):
            for block_id in range(self.n_blocks[i]):
                ...
                #print('test: ', self.input_size_multiplier)
                # Shared weights
    ...
    def fit(self, train_ts_loader, val_ts_loader=None, n_iterations=None, verbose=True, eval_steps=1):
        ...
        if n_iterations >0:
            ...
            return self.final_outsample_loss

    def fit_cross_validation(self, X_df, Y_df, n_validations=1, n_iterations=None, input_size_multiplier=7, output_size=24, 
                             verbose=True, eval_steps=1):
        if n_validations > 9:
            raise Exception("Number of iterations not valid")
            
        # implied input_size
        input_size=input_size_multiplier*output_size
        print('input size:', input_size)
        list_stacks = ['exogenous_tcn']+['trend']+['seasonality']
        best_losses = []
        
        for i in range(n_validations):
            data_size = len(X_df)-int(len(X_df) * (n_validations - i - 1) * 0.1)
            training_size = len(X_df)-int(len(X_df) * (n_validations - i) * 0.1)
            train_mask = np.ones(data_size)
            
            X_df_tmp = X_df[:data_size]
            Y_df_tmp = Y_df[:data_size]
            train_mask[training_size:] = 0
            ts_dataset = TimeSeriesDataset(Y_df=Y_df_tmp, X_df=X_df_tmp, ts_train_mask=train_mask)
            # To review how to partition this is why we're doing it here
            train_loader = TimeSeriesLoader(model='nbeats',
                                            ...
                                            idx_to_sample_freq=24, # this affects the size, the bigger less days
                                            batch_size=512,
            ...
            val_loader = TimeSeriesLoader(model='nbeats',
                                          ...
                                          input_size=input_size,
                                          output_size=output_size,
                                          idx_to_sample_freq=24, #this affects the size, the bigger less days 24
                                          batch_size=512,
                                          is_train_loader=False,
                                          shuffle=False)
            
            # Assume fit method returns best_val_loss when val_loader is set
            best_val_loss = self.fit(train_ts_loader=train_loader, val_ts_loader=val_loader, eval_steps=50)
            best_losses.append(best_val_loss)
            print('best_losses: ', best_losses)
        
        print('\n')
        print('='*30+' Best Loss functions '+'='*30)
        print(best_losses)
            
        return val_loader, np.mean(best_losses)
    




    def predict(self, ts_loader, X_test=None, return_decomposition=False, return_dates=False):
        self.model.eval()
        ...
        if return_dates:
            ...
            for batch in ts_loader:
                dates.extend(batch['dates'])  # Assuming 'dates' is the key for dates in the batch
            return y_true, y_hat, dates  # Return dates along with predictions
        else:
            return y_true, y_hat
